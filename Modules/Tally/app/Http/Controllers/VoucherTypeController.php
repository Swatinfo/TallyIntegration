...
use Modules\Tally\Http\Requests\StoreVoucherTypeRequest;
...
class VoucherTypeController extends Controller
{
    ...
    public function show(string $connection, string $name): JsonResponse
    {
        ...
        if (! $item) {
            return response()->json(['success' => false, 'data' => null, 'message' => 'Voucher type not found'], 404);
        }

        return response()->json(['success' => true, 'data' => $item, 'message' => 'Voucher type retrieved successfully']);
    }
    ...
    public function store(StoreVoucherTypeRequest $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => $result,
            'message' => $result['errors'] === 0 ? 'Voucher type created successfully' : 'Failed to create voucher type',
    ...
    public function update(Request $request, string $connection, string $name): JsonResponse
    {
        ...
        return response()->json([
            'success' => $result['errors'] === 0,
            'data' => $result,
            'message' => $result['errors'] === 0 ? 'Voucher type updated successfully' : 'Failed to update voucher type',
        ]);
    }
    ...
    public function destroy(string $connection, string $name): JsonResponse
    {
        ...
        return response()->json([
            'success' => $result['errors'] === 0,
            'data' => $result,
            'message' => $result['errors'] === 0 ? 'Voucher type deleted successfully' : 'Failed to delete voucher type',
        ]);
    }
}
