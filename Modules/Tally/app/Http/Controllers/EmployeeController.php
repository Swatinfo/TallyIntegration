...
use Modules\Tally\Http\Requests\StoreEmployeeRequest;
use Modules\Tally\Services\Concerns\PaginatesResults;
use Modules\Tally\Services\Masters\EmployeeService;
...
class EmployeeController extends Controller
{
    ...
    public function show(string $connection, string $name): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'data' => $item, 'message' => 'Employee retrieved successfully']);
    }
    ...
    public function store(StoreEmployeeRequest $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => $result,
            'message' => $result['errors'] === 0 ? 'Employee created successfully' : 'Failed to create employee',
    ...
    public function update(Request $request, string $connection, string $name): JsonResponse
    {
        ...
        return response()->json([
            'success' => $result['errors'] === 0,
            'data' => $result,
            'message' => $result['errors'] === 0 ? 'Employee updated successfully' : 'Failed to update employee',
        ]);
    }
    ...
    public function destroy(string $connection, string $name): JsonResponse
    {
        ...
        return response()->json([
            'success' => $result['errors'] === 0,
            'data' => $result,
            'message' => $result['errors'] === 0 ? 'Employee deleted successfully' : 'Failed to delete employee',
        ]);
    }
}
